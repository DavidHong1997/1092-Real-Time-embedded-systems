...
static  void  OS_SchededEDF(void);						/* Earlist deadline First Scheduling */
...
Node* ListInsertNode(Node *head, int state, int ticks, int prio_cur, int prio_next){
	Node *new_node = (Node*) malloc(sizeof(Node));
	new_node->state = state;
	new_node->ticks = ticks;
	new_node->prio_cur = prio_cur;
	new_node->prio_next = prio_next;
	new_node->next = NULL;
	if(head == NULL){
		head = new_node;
	}else{
    ...
    if (OSRunning == TRUE) {
        ...
        if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Reschedule only if all ISRs complete ... */
            ...
            if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy */
                ...
                /*                              Modification                                           */
	 	 	 	head = ListInsertNode(head, 1, (OSTime), OSTCBCur->OSTCBPrio, OSPrioHighRdy);
                /*                                END                                                  */
...
void  OSStart (void)
{
    if (OSRunning == FALSE) {
        ...
        //OSPrioHighRdy = (INT8U)((y << 3) + x);
	 	OS_SchededEDF(); 							 /* Go to EDF scheduling setup OSPrioHighRdy 	   */
...
void  OSTimeTick (void)
{
    ...
    OS_EXIT_CRITICAL();
#endif
    OSTCBCur->compTime--;                                  /*             Time consume                 */
    if (OSRunning == TRUE) {    
...
void  OS_Sched (void)
{
    ...
    if ((OSIntNesting == 0) && (OSLockNesting == 0)) { /* Sched. only if all ISRs done & not locked    */
	 	OS_SchededEDF();							   /* Go to EDF scheduling 						   */
        OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
        if (OSPrioHighRdy != OSPrioCur) {              /* No Ctx Sw if current task is highest rdy     */
            /*                                  Modification                                           */     
	 	 	head = ListInsertNode(head, 2, (OSTime), OSTCBCur->OSTCBPrio, OSPrioHighRdy);
            /*                                 END                                                     */   
...
*
*              comptime		is to record the compute time
*
*              deadline		EDF to search cur early deadline
*
*
...
INT8U  OS_TCBInit (INT8U prio, OS_STK *ptos, OS_STK *pbos, INT16U id, INT32U stk_size, void *pext, INT16U opt, INT32U compTime, INT32U deadline)
{
#if OS_CRITICAL_METHOD == 3                                /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr;
#endif    
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        ptcb->OSTCBId        = id;                         /* Store task ID                            */
	 	ptcb->compTime 		 = compTime;				   /* Store compTime						   */
	 	ptcb->deadline		 = deadline;				   /* Store deadline						   */
#else
