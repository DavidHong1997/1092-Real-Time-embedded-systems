...
INT8U  OSTaskCreate (void (*task)(void *pd), void *pdata, OS_STK *ptos, INT8U prio, INT32U compTime, INT32U deadline)
{
#if OS_CRITICAL_METHOD == 3                  /* Allocate storage for CPU status register               */
    OS_CPU_SR  cpu_sr;
#endif
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, (OS_STK *)0, 0, 0, (void *)0, 0, compTime, deadline);
        if (err == OS_NO_ERR) {
            OS_ENTER_CRITICAL();
                        ...
                        INT16U   opt)
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, pbos, id, stk_size, pext, opt, 10, 10); //defaut comptime = 10, deadline = 10
        if (err == OS_NO_ERR) {
            OS_ENTER_CRITICAL();
            OSTaskCtr++;                                       /* Increment the #tasks counter         */
            OS_EXIT_CRITICAL();
