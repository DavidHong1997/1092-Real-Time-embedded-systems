...
Node* ListInsertResource(Node *head, int state, int ticks, int prio_cur, int prio_next, int resource)
{
...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if ((INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8) == OS_MUTEX_AVAILABLE) {
        ...
        ptcb  = (OS_TCB *)(pevent->OSEventPtr);                       /*     Point to TCB of mutex owner   */
	 	...
	 	ticks = OSTime;
	 	resource = cpp;												  /*what Resource?                      */
	 	prio_cur = OSTCBCur->OSTCBPrio;

        if(cpp > ptcb->OSTCBPrio){
          Mutex_store[0] = cpp;                    /*Org cpp                             */
          cpp = ptcb->OSTCBPrio;
          pevent->OSEventCnt  = (cpp << 8) | OSTCBCur->OSTCBPrio;
          Mutex_store[1] = 1;                      /*Use Store index*/
        }
        ...
        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
	 	prio_next = OSTCBCur->OSTCBPrio;
	 	state = 1; 													  /* Lock index */ 	
        OSTCBPrioTbl[cpp]       = (OS_TCB *)ptcb;
	 	head2 = ListInsertResource(head2, state, ticks, prio_cur, prio_next, resource);
    }
}
